 ...
 * App.jsx imports it for submit; CardViewer.jsx imports it to rebuild a deal's cards.
...
// DTF Gang Sheet block: what the vinyl department needs to lay out and print the sheet -- sheet size,
// each graphic's size and how many of it, and the estimate the Deal's print count came from (D-13).
function pcGangSheetBlock(p, bi) {
  const gang = gangSheetPrints(p);
  const graphics = (p?.gangGraphics || []).map((g, i) => {
    const row = gang.perGraphic?.[i];
    const per = pcHas(g?.quantityPerSheet) ? g.quantityPerSheet : row ? `${row.placed} (fills the sheet)` : "";
    return `<div class="graphic"><div class="grtitle">${bi ? "Graphic / Gráfico" : "Graphic"} ${i + 1}${pcHas(g?.graphicDescription) ? ": " + pcEsc(g.graphicDescription) : ""}</div>${pcRow("Size", "Tamaño", pcHas(g?.graphicWidth) && pcHas(g?.graphicHeight) ? `${g.graphicWidth}" x ${g.graphicHeight}"` : "", { crit: true, bi })}${pcRow("Per Sheet", "Por Hoja", per, { crit: true, bi })}${row && row.rotated ? pcRow("Layout", "Disposición", "rotated 90° to fit more", { bi }) : ""}${pcRow("Colors", "Colores", g?.numberOfColorsUsed, { bi })}${pcRow("Colors Used", "Colores", g?.colorsUsed, { bi })}${pcRow("Print Ready?", "¿Listo?", g?.isGraphicPrintReady === true ? "Yes" : g?.isGraphicPrintReady === false ? "No" : "", { bi })}</div>`;
  }).join("");
  const warn = gang.overflow ? `<div class="alsonote"><b>Does not all fit on one sheet at these sizes — check before printing.</b></div>` : "";
  return `<div class="garment"><div class="ghead"><span class="gname">Gang Sheet ${pcHas(p?.gangSheetWidth) && pcHas(p?.gangSheetHeight) ? `${pcEsc(p.gangSheetWidth)}" x ${pcEsc(p.gangSheetHeight)}"` : ""}</span><span class="gqty">${bi ? "Sheets / Hojas" : "Sheets"}: ${pcEsc(gang.sheets)}</span></div>${pcRow("Estimated Prints", "Impresiones Est.", `${gang.printsPerSheet} per sheet · ${gang.total} total`, { crit: true, bi })}${graphics}${warn}${pcRow("Other Information", "Información Adicional", pcNote(p), { bi })}</div>`;
}
...
function buildProductionCards(data, counts) {
  ...
  products.forEach((p) => {
    ...
    else if (p.productType === "gangsheet") (p.gangGraphics || []).forEach((gr) => art.push({ src: `${p.productName} — ${gr?.graphicWidth || "?"}" x ${gr?.graphicHeight || "?"}"`, gr }));
